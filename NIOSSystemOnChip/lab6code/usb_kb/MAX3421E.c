...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	dataToWrite[0] = reg + 2;
	dataToWrite[1] = val;
	int returnCode = alt_avalon_spi_command(SPI_0_BASE, 0, 2, dataToWrite, 0, 0, 0);
	if (returnCode) {
		alt_printf("Error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (int i = 1; i < nbytes + 1; i++) {
		dataToWrite[i] = data[i- 1];
	}
	int returnCode = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, dataToWrite, 0, 0, 0);
	if (returnCode < 0) {
		alt_printf("Error");
	}
	return data + nbytes;
